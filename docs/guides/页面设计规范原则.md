...
| # | 不统一点 | 当前表现 | 建议统一方向 | 状态 |
|---|---|---|---|---|
| 1 | 页面底色策略 | Inbox 有明确硬编码底色（`#f5f5f7` / `#0b0b0f` 等）；Category/Routing 页面容器更偏 token 继承 | Dashboard 页面优先 token，硬编码仅用于局部特效 | 已确认 |
| 2 | 字体气质 | Inbox 大量 `uppercase + font-black`；其他页偏常规阅读体 | 主信息统一常规字重，重装饰字重只保留局部标签 | 待确认 |
| 3 | 卡片视觉等级 | Inbox 卡片、Category/Routing grid 卡片圆角与阴影体系不同 | 统一卡片等级（基础卡/强调卡）并复用 class 组合 | 待确认 |
| 4 | 列表形态 | Category list 是 Table；Routing list 是分段条目 | 定义“数据表型”和“规则条目型”两种合法列表模板 | 待确认 |
| 5 | 筛选控件样式 | Inbox 筛选 pills 风格独立；其他页用标准控件 | 抽离统一 FilterBar（含 pills/select/search） | 待确认 |
| 6 | 调试区视觉风格 | Routing 测试区使用大量蓝绿调试配色块 | 区分“生产界面”与“调试界面”，调试样式降噪 | 待确认 |
...
- 浅色背景：`--background: 0 0% 96%`（约等于 `#f5f5f5`）
- 深色背景：`--background: 240 20% 4%`（约等于 `#08080c`）

Inbox 现有硬编码底色（示例）：

- 页面根容器：`bg-[#f5f5f7] dark:bg-[#0b0b0f]`
- 顶部区域：`dark:bg-[#0b0b0f]/50`
- 结果区域：`dark:bg-[#0b0b0f]/40`

结论：

- Inbox 的底色硬编码和 token 语义色“接近但不完全相同”，不建议继续在页面内重复硬编码。
- Category/Routing 的页面容器不主动写死底色，更多继承布局层 `bg-background/*`，因此一致性更高。
- 统一原则确定为：页面基底使用 token（`bg-background`/`bg-background/*`），硬编码仅保留在局部视觉特效层（如拖拽遮罩、调试状态块等）。
